...
def VSM(Data, Query, IDF, TOP_K, TOP_P):
	...
	for query in Query:
		...
		wcount = numpy.zeros((query.shape[0],1))
		
		for i in xrange(TOP_P):
			index = qresult[i][0]
			wcount = numpy.add(wcount,Data[index])
		W = numpy.argmax(wcount, axis=0)
		#print wcount
		F = numpy.argmax(query, axis=0)
		if  F == W:
			wcount[W] = 0
			W = numpy.argmax(wcount, axis=0)

		#print numpy.argmax(query, axis=0), W, wcount[numpy.argmax(query, axis=0)],wcount[W]

		query = numpy.zeros((query.shape[0],1))
		query[W] = 1.0
		#print "new query: ", W
		qlen = numpy.sqrt(numpy.dot(query.T,query))
		pqresult = []
		for i in xrange(len(Data)):
			...
			sim = numpy.dot(data.T,query)/(qlen*dlen)
			#print (i,sim)
			pqresult.append((i,sim))
		pqresult = sorted(pqresult, key=lambda tup: tup[1], reverse=True)
		#print len(pqresult)
		#print len(qresult)
