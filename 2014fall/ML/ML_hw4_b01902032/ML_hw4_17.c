...
int main()
{
	...
	for(i=0;i<SIZE;i++)
		...
		{
			la=pow(10,k);
			tmpw=mult(tran(x,120,3),x,3,120,3);
			for(i=0;i<3;i++)
				tmpw[i][i]+=la;
			w=mult(inverse(tmpw,3,3), mult(tran(x,120,3),y, 3,120,1), 3, 3, 1);
			for(i=0;i<3;i++)
				printf("%lf ",w[i][0]);
			printf("\n");
		...
		aveEout+=Eout;
		printf("logla = %d, la = %lf, Eingm = %lf, Einval= %lf, Eout = %lf\n", Fk, Fla, Eingm/120,Einval/80, Eout/1000);
