...
Grid** pre_processing(Grid ** testmap)
{
	...
	for(i=1;i<=SIZE;i++)
	{
		for(j=1;j<=SIZE;j++)
		{
			if(map[i][j].brick==0)
			{
				...
				for(k=0;k<r-l+1;k++)
				{
					part+=map[i][j+k].ph*map[i][j+k].heu;

				}
				for(k=0;k<r-l+1;k++)
				{
					map[i][j+k].prob=map[i][j+k].ph*map[i][j+k].heu/part;
